// Public S3 surface; first user is `loom service serve` registering the
...
//! `loom service serve`: one ServiceRuntime per spec, one tokio task per
...
//! scope — optionally as a directed wake message if the job has a
//! `targetAgent`.
//!
