...
use context_layer_core::estimate_tokens;
...
impl WarmSummaryContextResource {
    ...
    pub fn persist(profile_dir: &Path, scope_id: &str, summary: &str) -> Result<()> {
        ...
        loom_platform::path::create_dir_all(&dir)
...
impl ContextResource for WarmSummaryContextResource {
    ...
    fn assemble(&self, ctx: &AssemblyContext<'_>) -> Result<Vec<PromptSection>> {
        ...
        Ok(vec![PromptSection::from_resource_uri(
            "warm_summary",
            "warm-summary",
            format!("summaries/{}.md", ctx.scope.id),
            format!(
...
mod tests {
    ...
    // (b') R1.2: skip-not-truncate contract via the shared test helper.
    #[test]
    fn warm_summary_honors_skip_not_truncate() {
        use context_layer_core::test_support::assert_skip_not_truncate;

        let dir = tempfile::tempdir().unwrap();
        let scope_id = "test_scope_skip_not_truncate";
        let summary_text = "SESSION INTENT: r1 contract\nKEY DECISIONS: skip, never truncate\n"
            .repeat(5);
        WarmSummaryContextResource::persist(dir.path(), scope_id, &summary_text).unwrap();
        ...
        // Tiny budget (1 token → 20% = 0) forces the skip path; the
        // helper asserts no section is ever truncated.
        assert_skip_not_truncate(&r, &mk_ctx(10_000), &mk_ctx(1));
    }

    // (b'') R1.1: warm summary sections carry a traceable Resource uri.
    #[test]
    fn warm_summary_section_carries_resource_uri() {
        let dir = tempfile::tempdir().unwrap();
        let scope_id = "test_scope_provenance";
        WarmSummaryContextResource::persist(dir.path(), scope_id, "summary body").unwrap();
        ...
        assert_eq!(
            ...
            context_layer_core::SectionSource::Resource {
                scheme: "warm-summary",
                uri: Some(format!("summaries/{scope_id}.md")),
            }
        );
    }
