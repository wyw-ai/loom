...
- Role: 交付 agent：基于 discovery 的交接产物完成实现、验证、推送、MR、review 回应、归档和 feedback 收口。
...
- 例外：如果最新触发来自 `actor_router`，且 message 只是 `ack`、`收到`、`继续等`、
  `继续等待`、`无新进展`、`仍在等待 reviewer/CI` 或同义短句，这是 router 的
  等待确认，不是新任务。此时禁止再 handoff `actor_router` 回报“等待中”；可直接结束。
