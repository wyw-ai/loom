...
- Cargo workspace 已 build：`cargo build --release -p loom -p loom-server`。
...
- `jq` 在 PATH 里。
- 端口 7900 可用（`local-up.sh` 默认值；`LOOM_E2E_PORT` 可改）。
...
| `run-spec-apply.sh` | M5/O6：从合成 lesson-plan 走 `action.request → action.response → loom spec apply`，断言 `spec.json` 落盘、bundle write 落盘、`agent.config.changed` 携带 `reload_epoch_ms`、outcome artifact 发出。|
...
scripts/e2e/local-up.sh nuke    # 停进程 + 删 /tmp/loom-e2e
...
1. **`adapter ready` 之后无任何 actor event**：99% 是 LLM 调用卡住或
   spec.json 配置不对。先 `tail /tmp/loom-e2e/logs/agent-host.log`，
   再 `ps aux | grep "claude --print"` 看 LLM 进程是否在跑。
2. **`spec apply` 报 "spec.json not found"**：`LOOM_AGENT_SPECS` 没
...
- [ ] 在 187 上做的事仅限：拉新分支 → `cargo build --release` → 替换二进制 →
      停旧 dev-helper / dev-helper-bridge 进程 → 启动 `loom-server` /
      `loom agent serve` / `loom service serve` → 跑 1.4b/1.4c 同款剧情。
