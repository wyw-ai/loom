...
joi --json handoff <self_actor_id> --in <canonical_thread_id> --message "继续处理 task #N"
...
4. root event 气泡显示关联的 task/thread 链接；Emma 在 channel turn 里只回复一条
   简短说明，后续工作转到 canonical thread。
5. Emma 在 task thread 里工作，上传 artifact，并用 `task/update --artifact-id`
   绑定到 task。
