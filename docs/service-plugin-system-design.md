...
当前 Loom 的核心模型已经把人、agent、service 都统一成 `Actor`。`am` 钉钉接入也是这样跑起来的：外部脚本以 `svc_am_bridge` 这个 service actor 写入 thread，再通过 `directed_to` 关系把问题交给目标 agent。

这条链路证明了协议模型是可行的，但实现仍是脚本化的：

- `am` 的状态、thread 映射、重试、回发都在 Python 脚本里。
- scheduler 这类时间驱动入口还没有一等抽象。
- agent runtime 已经有外置化的 `loom agent serve`，service 还没有对应的长期进程模型。
- server 仍不应该感知 `am`、cron、CI watcher 这些具体业务。

因此下一步不是把每个 service 写进 `loom-server`，而是新增一个外置 service/plugin host，让 server 继续保持协议枢纽职责。

## 2. 设计目标

1. `loom-server` 保持纯消息枢纽：持久化、ACL、scope fanout、actor-inbox。
2. `am`、scheduler、CI watcher 等主动入口以 service plugin 形式运行。
3. 所有 service 都以 service actor 身份接入 Loom，写入标准 `message`。
4. service 之间共享生命周期、配置、状态目录、重试、幂等、日志、`responds_to` 订阅等基础能力。
5. agent 与 service 分层清晰：service 负责外部世界和时间触发，agent 负责被 directed message 后的推理与回复。

## 3. 非目标

- 不把第三方插件动态加载进 `loom-server` 进程。
...
`loom service serve` 是一个长期进程。它从配置目录加载 service specs，为每个 service actor 打开连接，启动对应 plugin。plugin 只和 `ServiceRuntime` 交互，不直接拼底层 JSON-RPC。

## 5. Service 与 Agent 的区别

| 维度 | Agent | Service |
| --- | --- | --- |
| 触发来源 | `directed_to` 事件 | 外部系统、时间、webhook、轮询、Loom 事件 |
| 输出 | agent 回复、tool trace、action.request | 标准 Loom event、外部回发、directed message |
...
- Loom scope subscription consumer。
...
- `send_directed_message(target_actor, scope, text, meta)`
- `await_message_replies(parent_message_id, timeout) -> Stream<message>`
...
`await_message_replies` 内部基于 actor-inbox 上的 message delivery，不要求 plugin 自己轮询 timeline。`am` 可以在这个订阅原语上实现 `block_until_first()` helper；scheduler、webhook 等 plugin 可以选择消费或忽略这类响应流。

ServiceRuntime 不提供 polling `message.read` 的便利方法。所有“等回答”都建立在 §9.1 (parent message 不变量) + §9.2 (durable actor inbox) 之上，避免回到脚本轮询 timeline 的反模式。
...
```text
~/.local/share/loom/service-host/services/am_dingtalk_qa/thread-map.json
```

### 7.3 Loom 写入

每条外部消息转换成一次 directed message：

```json
{
  "target": "#chan_x:msg_root",
  "body": "Source: DingTalk bot message ...\n\nUser message:\n...",
  "audience": [
    { "kind": "actor", "id": "actor_qa" }
  ],
...
回发依赖 agent 回复 message 中的：

```json
{
  "parentMessageId": "<directed message id>"
}
```
...
- 到点向 agent directed message，让 agent 分析或生成报告。
...
| `targetAgent` | 否 | 设置后触发 directed message；不设置则纯 logging |
| `dedupeBy` | 否 | `payload_hash`（默认） / `source_message_id` / `none` |
...
sequenceDiagram
    ...
    alt new event
        Plugin->>Runtime: send_message(scope, body, directed_to?)
        Runtime->>Server: message.send
        opt targetAgent set
...
cursor 持久化到 `~/.local/share/loom/service-host/services/<sid>/cursors/<job_id>.json`，failover 后重启即可恢复（与 §9.2 durable inbox 同语义但局限在 plugin 私有状态，不进 server journal）。
...
```json
{
  ...
  "relations": [
    { "kind": "directed_to", "target": { "kind": "actor", "id": "actor_planner" } }
  ]
}
```
...
`dedupe_once(key)` 必须在 `send_message` **之前**调用：写 dedupe 行 → 写 event。倒序会留下窗口，让两次 tick 撞同一个 source state 时双发。

dedupe 存储与 cursor 同目录：`~/.local/share/loom/service-host/services/<sid>/dedupe.jsonl`，append-only。S1 可以做按时间窗口截断（默认保留 14 天）。
...
- **Fire-and-forget**（默认，几乎所有 watcher 类 job）：send message with directed message，立刻 return，tick 之间不持有 turn-level 状态。agent 的回复通过 §9.5 RespondsTo 反向投递落到 `svc_scheduler` 的 inbox，但 plugin 可以选择仅订阅 cron 调度、忽略 actor-inbox push，让 reply 只在 channel/thread 给人看。
- **Awaited**（少数报告类 job，如"每天 9 点跑总结"）：send message 后调用 `await_responds_to(trigger_source_id, timeout)`，把 agent 文本作为 job 输出（写日志、回写另一个 channel、触发下一步 job）。timeout 必须显式（推荐 ≤ 单次 tick 间隔的 1/2），超时按 retry policy 处理。
...
~/.local/share/loom/service-host/services/<service_id>/
  ...
  pending/             # 仅 awaited 模式使用：未完成 turn 的 (job_id, trigger_source_id, deadline)
  logs/
    scheduler.log
    job-<job_id>.log
```
...
**scheduler 用到的方法**：`actor_upsert` / `open_connection` / `ensure_channel_member` / `send_message` / `directed message` / `state_dir` / `dedupe_once` / `cursor_load|save` / retry+backoff+logging。
...
所有针对 trigger message 产生的 agent 回复都必须携带 `parentMessageId = trigger_message_id`。这是 service plugin 能存在的前提：

- 失去这条关系，service 只能 polling timeline 猜回答。
- 拥有这条关系，§6.3 的 `await_message_replies` 才能基于 actor-inbox 工作。

`loom agent serve` 的 flush path 会写 parent message；缺 trigger（agent 主动开口）时降级为普通消息。
...
- 已处理事件要能记录 delivery ack 或 cursor。

可以先做最小 API：

```text
inbox.list(actorId, state=pending, limit, cursor)
delivery.ack(sourceId, actorId)
```

### 9.3 Thread API ACL（已实现）

`message.send` 已有 scope ACL；现在 thread create/list/update/delete 四个 handler 也按 caller-actor 校验 channel membership：

- `thread/create`：caller 必须是目标 channel 的 member（公区 channel 短路），且
  `rootMessageId` 必须指向该 channel 公共区消息。
...
实现细节：`crates/server/src/subscribe.rs::bind_actor` 的 preempt 规则覆盖 `ActorKind::Agent | ActorKind::Service`。短期 `loom --as svc_xxx <subcmd>` 默认 `actorKind = Human`（见 `crates/cli/src/client.rs::open_connection`），不会误抢长期 service host 的 inbox。stale 分支无条件清理失效绑定，使 host 干净重启同样能立即接管。
...
§9.1 让 agent 回复一定带 parent message，§9.2 给 service plugin 一个可恢复的 inbox。server 在保存 reply message 时，会把它反向投递给 parent message 的原始 actor，service actor 离线后也能通过 `inbox.list` 补收。

**规则**：当 message 带 `parentMessageId=<X>` 时，server 必须把这条 message 也按 actor-inbox 路径推送给 message X 的原始 actor，并写一条 `Pending` delivery。

**实现要点**：

- 与 directed audience 复用同一段 ACL gate（私有 channel 必须是 member）与 `already_sent` 去重（含 speaker 自防自循环）。
- self-response（actor 回自己的 message）跳过：不写 delivery row，避免噪声。
- 跟 §9.2 同 mutation：`Mutation::DeliveryUpsert`，journal replay 路径不变。

**实现位置**：

- 投递规则：`crates/server/src/store.rs` 保存 message 后计算 directed audience + parent reply target。
- 在线推送：`crates/server/src/ws.rs::fanout` 复用 delivery target 集合，共用后续 ACL / dedup / send_to_actor。

**对 §6.3 `await_message_replies` 的影响**：service plugin 在线时收到 `message.created` actor-inbox push；离线/重启时通过 §9.2 的 `inbox.list` 补收同一条 message。两条路径的过滤条件都是 `message.parentMessageId == trigger_message_id`，runtime 端语义统一，不再需要 polling timeline。
...
状态不进 server journal，因为这是 connector 运行状态，不是协作事实。进入 Loom 的只有标准 event / delivery ack / artifact。

迁移说明：现有 `examples/am-loom-channel-bridge.py` 的用户在 `~/.config/aone-message-cli/loom-thread-map.json` 持有 thread map。S2 的 `am` plugin 启动时如检测到旧路径存在且新路径未初始化，一次性 copy 到 `~/.local/share/loom/service-host/services/<service_id>/thread-map.json` 并打印迁移日志，不删除原文件。
...
```sh
loom service serve [--specs <dir>] [--server <url>] [--allow-services a,b,c]
loom service validate <file>
loom service list             # 只读：扫 specs 目录
loom service log <service_id>

# 延后：
# loom service install <marketplace_id>     # S4
```
...
- ✅ §9.1 嵌入 runtime 回复补 `responds_to`（`flush_text_as_event` 反查 `Turn.trigger_source_id`）。
...
后续 §9.2（durable actor inbox）作为 S1 一部分推进，与 ServiceRuntime 的 `await_responds_to` 一同设计，避免在 plugin 形态明确前提前敲死 `inbox.list` / `delivery.ack` 的 schema。
...
- ✅ Service host 公共代码落到 `crates/cli/src/service/`（doc 允许 in-crate 路径，未抽独立 `loom-client` crate）。
- ✅ `ServiceRuntime` 暴露 actor_upsert / open_connection / ensure_channel_member / invite_member / create_thread / send_message / directed message / await_responds_to / state_dir / dedupe_once / cursor_load|save。
- ✅ §9.2 durable inbox：proto + server `inbox.list` 带游标分页 + caller-actor ACL，5 个单元测试。
- ✅ ServiceSpec proto 类型 + 校验（actor.kind = service 强制），6 个单元测试。
- ✅ `loom service serve | validate` CLI 子命令；S1 不带任何内置 plugin（specs 的 kind 没匹配的 plugin 时记 warn 跳过），保留 host 监督壳。
...
- ✅ AmConfig 解析 ServiceSpec.config，env 配置全数迁到 spec.config（迁移表见 `docs/am-loom-bridge.md` §6）。
- ✅ `loom service am-handler --service-id <id>` CLI 子命令；从 stdin 读消息 → 解析 → 解析 scope → directed message → 三种 reply 模式（callback / send / async_send）。
- ✅ 旧 thread-map 一次性迁移：检测 `~/.config/aone-message-cli/loom-thread-map.json` 存在且新路径未初始化，copy 到 `<state_dir>/thread-map.json`，原文件保留。
- ✅ `examples/am-loom-channel-bridge.py` 标记 deprecated（顶部注释指向 Rust handler）。
- ✅ `docs/am-loom-bridge.md` 重写：换成 ServiceSpec 配置 + `am listen --script "loom service am-handler ..."`。
...
- ✅ `SchedulerPlugin` 长进程 loop：每个 job 一个 tokio task，按 cron tick 唤醒 → exec source → cursor diff → dedupe_once → directed message/append → 可选 `await_responds_to`（`scheduler/plugin.rs`，7 个测试）。
...
- ⚠️ §8.3 表里的 `cursorBy: jq:<expr>` 当前未实现（仅 `none` / `body_hash`）；schema 留着便于后续平滑加。`dedupeBy: source_message_id` 走 `payload_hash` 的实现路径，理由同前。
...
Service plugin 系统的核心不是“插件加载”，而是把外部世界进入 Loom 的路径标准化：

```text
external event / time event
  -> service actor
  -> standard Loom message
  -> optional directed_to agent
  -> agent responds_to trigger
  -> optional external reply
```

只要这条链路稳定，`am`、scheduler、CI watcher、需求同步、告警接入都可以在同一个模型里扩展，而 `loom-server` 不需要知道任何具体外部系统。
