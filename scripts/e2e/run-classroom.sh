...
root=$(thread_anchor "$ch" "anchor: rename-lesson-target")
th=$(j thread create --channel "$ch" --root-event "$root" --as actor_e2e_human --title "rename-lesson-target" | jq -r '.thread.id')
echo "channel = $ch"
echo "thread  = $th"
